            ...
            <div class="px-6 py-5 bg-gradient-to-r from-gray-50 to-white dark:from-gray-900/60 dark:to-gray-800 border-b dark:border-gray-700">
                <div class="flex items-center gap-5">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Daily rate: <span class="font-semibold text-[#800020] dark:text-[#FFD600]">₦{{ number_format($bike->daily_rate, 2) }}</span> per day</p>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('home.bikes.hire.store', $bike->slug) }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="pickup_address" rows="2"
                              class="w-full rounded-xl border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-2 focus:ring-[#800020] focus:border-transparent transition-all py-2.5 px-4">{{ old('pickup_address') }}</textarea>
                </div>
