                                ...
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                            ...
                                            <span class="font-mono tracking-wide">#{{ $tracking }}</span>
                                        </p>
                                        @endif
                                    </div>
                                    <div class="mt-4 md:mt-0">
                                        <span class="block text-sm text-gray-600 dark:text-gray-400">Total Amount</span>
                                        <span class="text-2xl font-bold text-gray-900 dark:text-white">{{ $settings->currency }}{{ number_format($prefilled_amount ?? 0, 2) }}</span>
                                    </div>
                                </div>
                            </div>

                            <form method="POST" action="{{ route('home.process-deposit') }}" class="space-y-6">
                                @csrf
                                <input type="hidden" name="asset" value=" ">
                                @if(isset($tracking))
                                <input type="hidden" name="tracking" value="{{ $tracking }}">
                                @endif                                <!-- Payment Method Selection -->
                                <div class="space-y-3">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        @forelse ($dmethods as $index => $method)
                                            <div class="relative">
                                                <input type="radio" id="method-{{ $index }}" name="payment_method" value="{{ $method->name }}" 
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 flex items-center pl-4">
                                            <span class="text-gray-500 dark:text-gray-400 font-medium">{{ $settings->currency }}</span>
                                        </div>
                                        <input type="number"
                                               id="amount"
                                               name="amount"
                                               ...
                                               step="0.01"
                                               placeholder="0.00"
                                               value="{{ $prefilled_amount ?? '' }}"
                                               {{ $prefilled_amount ? 'readonly' : '' }}
                                               class="block w-full pl-10 pr-4 py-3.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700
                                                     ...
                                                     {{ $prefilled_amount ? 'bg-gray-50 dark:bg-gray-900' : '' }}">
                                    </div>
                                    @if(!$prefilled_amount)
                                        ...
                                        @foreach($quickAmounts as $amount)
                                            <button type="button" 
                                                    ...
                                                    class="px-3 py-1.5 text-xs font-medium bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700
                                                        ...
                                                        transition-colors">
                                                {{ $settings->currency }}{{number_format($amount)}}
                                            </button>
                                        @endforeach
                                    </div>
                                    @endif
