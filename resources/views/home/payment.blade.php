...
<div class="min-h-screen bg-white dark:bg-gray-900" x-data="paymentHandler()">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 relative z-10">
            <div class="text-center">
                ...
                <p class="text-blue-100 text-lg max-w-2xl mx-auto">
                    Pay securely via <span class="font-semibold">{{ $payment_mode->name }}</span> to clear your shipment from customs and continue delivery
                </p>
            </div>
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 -mt-8 relative z-20">
        <div class="space-y-4 mb-6" x-data="{ showSuccess: {{ session('success') ? 'true' : 'false' }}, showError: {{ session('error') ? 'true' : 'false' }} }">
            <!-- Custom Error Alert -->
        ...
        <form method="POST" enctype="multipart/form-data" action="{{ route('home.process-payment') }}" class="space-y-8">
            @csrf
                        ...
                        <div class="p-6 space-y-6">
                                ...
                                <div class="flex flex-col md:flex-row justify-between items-start md:items-center">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                            ...
                                            <span class="font-mono tracking-wide">{{ session('tracking') }}</span>
                                        </p>
                                        @endif
                                    </div>
                                    <div class="mt-4 md:mt-0">
                                        <span class="block text-sm text-gray-600 dark:text-gray-400">Total Amount</span>
                                        <span class="text-2xl font-bold text-gray-900 dark:text-white">{{ $settings->currency }}{{ number_format($amount, 2) }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center space-x-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                                    ...
                                    <div>
                                        <h4 class="font-medium text-gray-900 dark:text-white">{{ $payment_mode->name }}</h4>
                                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $payment_mode->duration ?? 'Processing time: 1-24 hours' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative">
                                    ...
                                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                                        Please send exact amount: {{ $settings->currency }}{{ number_format($amount, 2) }}
                                    </p>
                                </div>
                            </div>
                            ...
                            @if(session()->has('tracking'))
                            <input type="hidden" name="tracking" value="{{ session('tracking') }}">
                            @endif
                            @if($asset)
