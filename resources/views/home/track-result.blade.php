            ...
            <div x-show="!loading" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    <span class="text-2xl md:text-3xl font-mono font-bold text-white">{{ $courier->trackingnumber }}</span>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <span class="px-3 py-1 rounded-full text-sm font-semibold {{ $courier->status === 'Delivered' ? 'bg-green-500 text-white' : ($courier->status === 'Custom Hold' ? 'bg-amber-500 text-white' : 'bg-blue-500 text-white') }}">
                                    {{ $courier->status }}
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 border-b border-gray-100">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ $courier->weight }} kg</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-[#800020]/10 rounded-xl flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ $courier->freight_type }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-[#800020]/10 rounded-xl flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ \Carbon\Carbon::parse($courier->date_shipped)->format('M d, Y') }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-[#800020]/10 rounded-xl flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-800">{{ \Carbon\Carbon::parse($courier->expected_delivery)->format('M d, Y') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="absolute bottom-4 left-4 bg-white/95 backdrop-blur-sm p-3 rounded-xl shadow-md max-w-xs">
                                    ...
                                    <div class="space-y-2 text-xs">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-gray-600">Current: {{ $courier->location }}</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                            ...
                            <div class="p-5 max-h-[450px] overflow-y-auto">
                                @if($tracks->count() > 0)
                                    <div class="relative pl-4 border-l-2 border-gray-200 space-y-5">
                                        @foreach($tracks->sortByDesc('created_at') as $track)
                                            <div class="relative">
                                                <div class="absolute -left-[9px] top-0 w-4 h-4 rounded-full {{ ($track->status === 'Delivered') ? 'bg-green-500' : (($track->status === 'Custom Hold') ? 'bg-amber-500' : 'bg-[#800020]') }} border-2 border-white shadow-sm"></div>
                                                <div class="ml-4">
                                                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($track->created_at)->format('M d, Y - h:i A') }}</p>
                                                    <p class="text-sm font-semibold text-gray-800 mt-0.5">{{ $track->status }}</p>
                                                    @if($track->city)
                                                        <p class="text-xs text-gray-500 mt-0.5">{{ $track->city }}</p>
                                                    @endif
                                                    @if($track->comment)
                                                        <p class="text-xs text-gray-600 mt-1">{{ $track->comment }}</p>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div class="bg-white rounded-xl shadow-sm p-4 border-l-4 border-blue-500">
                        ...
                        <p class="text-gray-800 text-sm font-medium">{{ $courier->sname }}</p>
                        <p class="text-gray-500 text-xs mt-1">{{ $courier->saddress }}</p>
                    </div>
                    <div class="bg-white rounded-xl shadow-sm p-4 border-l-4 border-[#FFD600]">
                        ...
                        <p class="text-gray-800 text-sm font-medium">{{ $courier->location }}</p>
                        @if($tracks->count() > 0)
                            <p class="text-gray-400 text-xs mt-1">Updated: {{ \Carbon\Carbon::parse($tracks->sortByDesc('created_at')->first()->created_at)->format('M d, h:i A') }}</p>
                        @endif
                    </div>
                    <div class="bg-white rounded-xl shadow-sm p-4 border-l-4 border-green-500">
                        ...
                        <p class="text-gray-800 text-sm font-medium">{{ $courier->name }}</p>
                        <p class="text-gray-500 text-xs mt-1">{{ $courier->address }}</p>
                        <p class="text-gray-500 text-xs">{{ $courier->phone }}</p>
                    </div>
                </div>

                <!-- Actions -->
                <div class="flex flex-wrap gap-3 justify-center">
                    <a href="{{ route('printnow', $courier->id) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#800020] text-white rounded-full text-sm font-semibold hover:bg-[#6b001a] transition-all shadow-sm">
                        <i class="fas fa-print"></i> Print Receipt
                    </a>
                    @if(($courier->clearance_cost ?? 0) > 0 || ($courier->cost ?? 0) > 0)
                    <a href="{{ route('home.deposits', ['courier_id' => $courier->id]) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#FFD600] text-[#800020] rounded-full text-sm font-semibold hover:bg-[#e6c200] transition-all shadow-sm">
                        <i class="fas fa-credit-card"></i> Pay Fee (₦{{ number_format(($courier->clearance_cost ?? 0) + ($courier->cost ?? 0), 2) }})
                    </a>
                    @endif
                </div>
            </div>
