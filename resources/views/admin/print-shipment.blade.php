...
<body>
    <div class="invoice-container">
        <div class="invoice-header">
            <h1 class="invoice-title">{{ $settings->site_name }} Logistics</h1>
        ...
        <div class="invoice-body">
            <div class="tracking-info">
                <div class="barcode">
                    <img src="https://barcode.tec-it.com/barcode.ashx?data={{ $shipment->trackingnumber }}&code=Code128" alt="{{ $shipment->trackingnumber }}">
                </div>
                <div class="tracking-number">{{ $shipment->trackingnumber }}</div>
                <div class="tracking-status status-{{ strtolower(str_replace(' ', '-', $shipment->status)) }}">
                    {{ $shipment->status }}
                </div>
            </div>
            ...
            <div class="shipment-details">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>Date Created</td>
                            <td>{{ $shipment->created_at->format('F d, Y') }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <table class="cost-table">
                <thead>
                    <tr>
                        ...
                        <th class="text-right">Amount ({{ $settings->s_currency }})</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Shipping Cost</td>
                        <td class="text-right">{{ number_format($shipment->cost, 2) }}</td>
                    </tr>
                    <tr>
                        <td>Clearance Cost</td>
                        <td class="text-right">{{ number_format($shipment->clearance_cost, 2) }}</td>
                    </tr>
                    <tr class="total">
                        <td>Total</td>
                        <td class="text-right">{{ number_format($shipment->cost + $shipment->clearance_cost, 2) }}</td>
                    </tr>
                </tbody>
            </table>
